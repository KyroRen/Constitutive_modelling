...
class UnifiedMaterialSolverMultiAxial:
    ...
    """
    def __init__(self, E, nu, yield_stress, material_model, method='implicit', precision='standard'):
        """
    ...
    def set_method(self, method='implicit', precision=None):
        """
        Switch between explicit and implicit methods.
        """
    ...
    def compute_stress(self, total_strain_tensor, n_substeps=1):
        """
        Compute stress using the selected method (explicit or implicit).
        """
        if self.method == 'explicit':
            return self._compute_stress_explicit(total_strain_tensor, n_substeps)
        else:  # implicit
            return self._compute_stress_implicit(total_strain_tensor, n_substeps)
    ...
    def _compute_stress_implicit(self, total_strain_tensor, n_substeps=1):
        """
        Implicit stress computation with adaptive yield point detection.
        """
        # Check if we're crossing the yield point
        elastic_strain_trial = total_strain_tensor - self.plastic_strain
        trace_e_trial = np.trace(elastic_strain_trial)
        ...
        if f_trial > 1e-8 and hasattr(self, '_last_stress_tensor'):
            ...
            if prev_f <= 1e-8:
                ...
                if exact_yield_strain_tensor is not None:
                    # Compute stress at exact yield point first
                    yield_stress = self._compute_stress_internal_implicit(exact_yield_strain_tensor, n_substeps)
                    self._yield_point_stress = yield_stress
        
        # Now compute the actual stress for the target strain
        result = self._compute_stress_internal_implicit(total_strain_tensor, n_substeps)
        self._last_stress_tensor = result.copy()
        return result
    ...
    # Removed _apply_plastic_strain_smoothing method
    
    def _compute_stress_internal_implicit(self, total_strain_tensor, n_substeps=1):
        """
        Internal stress computation method with robust root-finding approach.
        """
        d_total_strain = total_strain_tensor - self.total_strain
        ...
        for _ in range(n_substeps):
            ...
            if f_sub > self._yield_tolerance:
                ...
                def residual_coupled(x):
                    ...
                    # Residual Eq 1: Backstress Evolution (discretized physical law)
                    # THIS IS NOW GENERIC AND CALLS THE MATERIAL MODEL'S IMPLICIT METHOD
                    alpha_old_list = self.backstress
                    alpha_updated_list = self.material_model.update_backstress_implicit(alpha_old_list, dp_guess, flow_direction)
                    
                    residuals_alpha = []
                    for i in range(self.n_components):
                        # The residual is the difference between the solver's guess and the model's calculation
                        res = alpha_new_list[i] - alpha_updated_list[i]
                        residuals_alpha.append(res.flatten())
                    res_alpha = np.concatenate(residuals_alpha)

                    # Residual Eq 2: Yield Condition
                    total_backstress_new = np.sum(alpha_new_list, axis=0)
                    R_new, _ = self.material_model.update_state_vars_implicit(self.R, self.p, dp_guess)
                    d_epsilon_p = np.sqrt(3./2.) * dp_guess * flow_direction
                    stress_new = trial_stress_sub - 2 * self.mu * d_epsilon_p
                    res_f, _ = self.material_model.yield_function(stress_new, total_backstress_new, self.sigma_y + R_new)
                ...
                x0[0] = f_initial_abs / (self.E + h_plastic) if (self.E + h_plastic) > 1e-6 else 0.0

                for i in range(self.n_components):
                    x0[1 + i*9 : 1 + (i+1)*9] = self.backstress[i].flatten()

                # Use a more robust solver like 'hybr' which doesn't require Jacobian
                solution = root(residual_coupled, x0, method='hybr', tol=1e-8)
                
                if solution.success:
                    # Plastic step was successful
                    ...
                    # Record the equivalent plastic strain increment
                    # final_dp is the correct equivalent plastic strain increment for any loading condition
                    dp_total_step += final_dp
                else:
                    # Fallback: If solver fails, try with a smaller step size
                    # print(f"Warning: Solver failed at substep. Using elastic step.")
                    self.current_stress = trial_stress_sub
                    # Add a small dp to maintain continuity
                    dp_total_step += 1e-12
            else:
    ...
    def run_strain_controlled(self, strain_history_tensors):
        ...
        for i in range(len(strain_history_tensors)):
            ...
            try:
                correct_e_t = newton(transverse_residual, x0=previous_e_t, tol=1e-8, maxiter=20)
            except RuntimeError:
                correct_e_t = -nu * target_e11

            # Restore state for final computation
    ...
    def get_method_info(self):
        ...
        return {
            ...
            'available_methods': ['explicit', 'implicit'],
...
class UnifiedMaterialSolverUniAxial:
    ...
    def _solve_plastic_multiplier_fully_implicit(self, trial_stress, total_backstress_old, sign, R_old):
        ...
        def residual(dp):
            ...
            # 1. Update Backstress IMPLICITLY
            # This calls the method that solves the local Newton loop for alpha_new(dp)
            if hasattr(self.material_model, 'update_backstress_implicit'):
                new_backstress = self.material_model.update_backstress_implicit(self.backstress, dp_reg, sign)
            else:
                # Fallback to explicit if model doesn't support implicit (though it should for this mode)
                temp_state = {'sign': sign, 'R': R_old, 'p': getattr(self, 'p', 0.0)}
                new_backstress = self.material_model.update_backstress(self.backstress, dp_reg, sign, temp_state)

            # 2. Update Isotropic Hardening IMPLICITLY
            # Assuming isotropic model structure supports this (Voce does)
            new_R, _ = self.material_model.isotropic_model.update_implicit(R_old, getattr(self, 'p', 0.0), dp_reg)
        ...
        # --- Solver Strategy (same as robust solver) ---
        f_initial, _ = self.material_model.yield_function(trial_stress, total_backstress_old, self.sigma_y + R_old)
        
        # Initial guess
        state_vars_dummy = {'sign': sign, 'R': R_old, 'p': getattr(self, 'p', 0.0)}
        plastic_modulus = self.material_model.compute_plastic_modulus(self.backstress, state_vars_dummy)
        effective_modulus = self.E + max(plastic_modulus, 1000)
        initial_guess = max(f_initial / effective_modulus, 1e-12)

        # Try Newton
        try:
            from scipy.optimize import newton
            dp_solution = newton(residual, x0=initial_guess, tol=1e-10, maxiter=20)
            if dp_solution >= 0 and abs(residual(dp_solution)) < 1e-8:
                return dp_solution
        except:
            pass
            
        # Try Brentq (Bracketing)
        ...
        try:
                ...
                for _ in range(50):
                    ...
                    if val > prev_val:
                        # We passed the minimum! The root must be in [prev_upper, upper]
                        ...
                        # If still not found, try minimizing explicitly?
                        # Or assume min is positive (no solution -> elastic fallback?)
                        # But we checked f_initial > 0, so elastic is not valid.
                        # Maybe we are just in a local noise region? 
                        # Continue expanding cautiously
                    ...
                    upper *= 2.0
                     ...
                     raise RuntimeError(f"Bracket not found. f(0)={residual(0):.2e}, f({upper:.2e})={residual(upper):.2e}")

                if not bracket_found:
                     # One last ditch effort: minimize
                     # from scipy.optimize import minimize_scalar
                     # res = minimize_scalar(lambda x: abs(residual(x)), bounds=(0, upper), method='bounded')
                     # if residual(res.x) <= 0: upper = res.x ...
                     raise RuntimeError("Bracket not found after refinement.")

                lower = 0.0
                dp_solution = brentq(residual, lower, upper, xtol=1e-12, maxiter=50)
                if dp_solution >= 0:
                    return dp_solution
        except Exception as e:
    ...
    def _solve_plastic_multiplier_robust(self, trial_stress, total_backstress, sign, R):
        ...
        # Strategy 1: Simple Newton's method with robust fallback
        try:
            from scipy.optimize import newton
            dp_solution = newton(residual, x0=initial_guess, tol=1e-10, maxiter=20)
            if dp_solution >= 0 and abs(residual(dp_solution)) < 1e-8:
        ...
        # Strategy 2: Try with different initial guesses
        for scale in [0.5, 1.0, 2.0]:
            try:
                guess = initial_guess * scale
                dp_solution = newton(residual, x0=guess, tol=1e-10, maxiter=15)
                if dp_solution >= 0 and abs(residual(dp_solution)) < 1e-8:
                    return dp_solution
            except:
                continue

        # Strategy 3: Robust bracketing solver (brentq) instead of explicit fallback
        # This prevents numerical jumps by ensuring a consistent implicit solution.
        from scipy.optimize import brentq
        try:
            # Find a valid bracket [a, b] where residual(a) and residual(b) have opposite signs
            lower_bound = 0.0  # dp cannot be negative
            if residual(lower_bound) > 0:
                # Search for an upper bound where the residual becomes negative
                upper_bound = 2 * initial_guess
                for _ in range(15):
                    if residual(upper_bound) <= 0:
                        break
                    upper_bound *= 2
                else:
                    # If no bracket is found, the solver fails.
                    raise RuntimeError("Failed to find a valid bracket for brentq.")

                dp_solution = brentq(residual, lower_bound, upper_bound, xtol=1e-12, maxiter=100)
                if dp_solution >= 0:
                    return dp_solution
        except Exception as e:
            # If brentq fails, raise the error to be handled by the caller.
            # This is better than returning an inconsistent result.
            pass
    ...
    def compute_strain(self, target_stress, tol=None, max_iter=None):
        ...
        # Try multiple Newton attempts with different starting points
        newton_attempts = [
            initial_guess_strain,
            delta_strain_elastic,
            initial_guess_strain * 0.5,
            initial_guess_strain * 1.5,
            initial_guess_strain * 0.1,
            delta_strain_elastic * 0.5,
            delta_strain_elastic * 1.5,
            delta_strain_elastic * 2.0
        ]
        
        # **UNIFIED: Use same multi-method approach as multiaxial solver**
        from scipy.optimize import root
        
        for attempt_idx, guess in enumerate(newton_attempts):
            try:
                # Use multiple methods and tolerance levels for robustness (same as multiaxial)
                for method in self._stress_control_methods:
                    for tolerance in self._stress_control_tolerance_levels:
                        try:
                            # Convert to array for root function (consistent with multiaxial)
                            guess_array = np.array([guess])
                            def residual_array(x):
                                return np.array([residual(x[0])])
                            
                            solution = root(residual_array, guess_array, tol=tolerance, method=method)
                            
                            if solution.success and np.max(np.abs(solution.fun)) < tolerance * 10:
                                final_delta_strain = solution.x[0]
                                # Verify the solution is reasonable
                                if abs(final_delta_strain) < 0.1:  # Reasonable strain increment limit
        ...
        # **UNIFIED: Enhanced fallback strategy (same as multiaxial solver)**
        if not solution_converged:
            ...
            if stress_diff < 1.0:  # Very small stress increment
                ...
                # Try with progressively smaller initial guesses (using same root method)
                for scale in [0.1, 0.05, 0.01]:
                    try:
                        scaled_guess = initial_guess_strain * scale
                        scaled_guess_array = np.array([scaled_guess])
                        def residual_array(x):
                            return np.array([residual(x[0])])
                        
                        solution = root(residual_array, scaled_guess_array, tol=self._stress_control_tolerance_levels[-1], 
                                      method='hybr')
                ...
                if not solution_converged:
                    # **CRITICAL FIX: Better fallback that maintains plastic continuity**
                    # Instead of pure elastic fallback, use a more sophisticated approach
                    
                    # Try to find a physically reasonable solution using smaller steps
                    best_delta_strain = None
                    best_residual = float('inf')
                    
                    # Try multiple elastic-based guesses with different scaling
                    for scale in [0.3, 0.5, 0.7, 0.9, 1.0, 1.2, 1.5]:
                        try:
                            test_strain = delta_strain_elastic * scale
                            test_residual = abs(residual(test_strain))
                            
                            if test_residual < best_residual:
                                best_residual = test_residual
                                best_delta_strain = test_strain
                        except:
                            continue
                    
                    if best_delta_strain is not None and best_residual < abs(delta_stress):
                        # Use the best physically reasonable guess
                        final_delta_strain = best_delta_strain
                        print(f"Converged to physical solution at stress {target_stress:.2f} MPa (residual: {best_residual:.2e})")
                    else:
                        # **CRITICAL FIX: Avoid pure elastic fallback that causes zigzag**
                        # Instead of using elastic prediction, use the best available guess
                        final_delta_strain = best_delta_strain if best_delta_strain is not None else delta_strain_elastic * 0.8
                        print(f"Warning: Using improved fallback at stress {target_stress:.2f} MPa")
